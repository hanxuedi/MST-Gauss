...
'''
将动态3D高斯模型按时间步导出为点云数据，生成包含位置、颜色、透明度等属性的PLY文件
使用方法：python export_perframe_3DGS.py --iteration 14000 --configs arguments/dnerf/lego.py --model_path output/dnerf/lego 
'''
...
'''
通过 GaussianModel 加载预训练的3D高斯模型（含球谐系数、透明度等参数）
使用 Scene 类管理场景数据，包括测试视角的相机参数
'''
...
'''
index:时间步；
viewpoint：该时间步下的相机视角，且viewpoint自带time属性，调用Gaussian._deformation(means3D, scales, 
                                                                 rotations, opacity, shs,
                                                                 time)即可获得变形后的高斯
调用 get_state_at_time 获取每个时间步高斯参数，生成ply文件
'''
