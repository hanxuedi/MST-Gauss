...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    '''
    核心代码：
    1. 数据采样（随机或按时间戳），得到viewpoint_stack及其temp_list
    2. 多视角渲染
    3. 损失计算（L1 + 正则化）
    4. 反向传播与参数更新
    5. 点云稠密化/剪枝
    6. 检查点保存与可视化
    功能：
    集成 network_gui 模块，支持通过GUI实时查看训练渲染结果
    支持导出训练过程的时序渲染视频
    '''
    for iteration in range(first_iter, final_iter+1):        
        ...
        while network_gui.conn != None:
            try:
                ...
                if custom_cam != None:
                    count +=1
                    # 乒乓循环逻辑：每遍历完一轮视频相机列表后，反向遍历（形成来回扫描效果）
                ...
                # 若GUI发送停止指令（keep_alive=False）或达到最大迭代次数，退出当前交互循环
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter :
                ...
                # 更新可见高斯点在图像空间中的最大半径，用于后续剪枝判断（过小或不可见点将被移除）
                ...
                阶段	不透明度阈值 (opacity_threshold)	梯度阈值 (densify_threshold)
                ----------------------------------------------------------------------
                Coarse	固定值 (opacity_threshold_coarse)	固定值 (densify_grad_threshold_coarse)
                Fine	线性递减 (初始→最终)	线性递减 (初始→最终)
                ...
                # 致密化条件：迭代步 > densify_from_iter 且为 densification_interval 整数倍，点云数 < 360k
                ...
                # 剪枝条件：迭代步 > pruning_from_iter 且为 pruning_interval 整数倍，点云数 > 200k
                ...
                # 生长条件：启用 add_point 且点云数 < 360k
                # 操作：在边缘或高梯度区域插入新点，补充几何细节（如物体轮廓）
...
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname):
    ...
    '''
    两阶段训练
    Coarse阶段：快速初始化高斯分布，捕捉场景大体结构。
    Fine阶段：优化细节，引入时间平滑性约束（time_smoothness_weight）处理动态场景。
    '''
