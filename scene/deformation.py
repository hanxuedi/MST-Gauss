...
class Deformation(nn.Module):
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        # 特征提取
        hidden = self.query_time(rays_pts_emb, scales_emb, rotations_emb, time_feature, time_emb) # 融合空间网格特征与时间编码
...
class deform_network(nn.Module):
    def __init__(self, args) :
        ...
        timenet_output = args.timenet_output    # 32
        grid_pe = args.grid_pe # 0
        times_ch = 2*timebase_pe+1 #9
        
        self.timenet = nn.Sequential(
        nn.Linear(times_ch, timenet_width), nn.ReLU(), # 9 --> 64
        nn.Linear(timenet_width, timenet_output))   # 64-->32
    ...
    核心代码：
    1. 对pos进行编码
    2. 过一个self.deformation_net，得到形变后的 means3D, scales, rotations, opacity, shs（没用）
    '''
    def forward_dynamic(self, point, scales=None, rotations=None, opacity=None, shs=None, times_sel=None):
        # times_emb = poc_fre(times_sel, self.time_poc)
        # 特征编码
        point_emb = poc_fre(point,self.pos_poc) #  位置编码
