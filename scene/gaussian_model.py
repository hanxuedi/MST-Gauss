...
class GaussianModel:
    ...
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            ...
            actual_covariance = L @ L.transpose(1, 2)  # 构造正定协方差矩阵
            symm = strip_symmetric(actual_covariance)
            return symm
        
        self.scaling_activation = torch.exp # 使用指数函数（torch.exp）确保缩放值为正数
        self.scaling_inverse_activation = torch.log # 对数函数（torch.log），用于参数反变换

        self.covariance_activation = build_covariance_from_scaling_rotation # 通过旋转矩阵和缩放矩阵计算协方差

        self.opacity_activation = torch.sigmoid # 不透明度使用sigmoid函数约束在(0,1)范围
        self.inverse_opacity_activation = inverse_sigmoid # 旋转参数通过归一化保证单位四元数有效性
    ...
    def __init__(self, sh_degree : int, args):
        ...
        self._xyz = torch.empty(0)      # 高斯位置
        self._deformation = deform_network(args)    # 变形场
        self._features_dc = torch.empty(0)  # 高斯直流球谐函数系数
        self._features_rest = torch.empty(0) # 高斯交流球谐函数系数
        self._scaling = torch.empty(0)      # 高斯缩放系数
        self._rotation = torch.empty(0)     # 高斯四元数
        self._opacity = torch.empty(0)      # 高斯不透明度
        self.max_radii2D = torch.empty(0)   # 2D投影最大半径，用于屏幕空间剔除
        self.xyz_gradient_accum = torch.empty(0)    # 位置梯度累积值
        self.denom = torch.empty(0)     # 累积次数
        self.optimizer = None       # 优化器
        self.percent_dense = 0  # 致密化百分比
        self.spatial_lr_scale = 0 # 位置学习率衰减系数
        self._deformation_table = torch.empty(0)    # _deformation_table标记可形变点
        self.setup_functions()

    '''
    模型快照​​
    capture()序列化所有可训练参数和优化器状态，包含
    '''
    def capture(self):
        return (
    ...
    def restore(self, model_args, training_args):
        ...
        self._deformation.load_state_dict(deform_state) #  加载形变网络
        self.training_setup(training_args)   # 重建优化器
    ...
    '''
    获取形变场（Deformation Field）的 轴对齐包围盒 (Axis-Aligned Bounding Box, AABB)
    确定点云在空间中的有效范围，确保新增点不会超出形变场的作用区域
    xyz_max：包围盒的最大坐标 (x_max, y_max, z_max)
    xyz_min：包围盒的最小坐标 (x_min, y_min, z_min)
    '''
    @property
    def get_aabb(self):
    ...
    '''
    为选中的点生成带有随机扰动的候选新点，并过滤超出包围盒的无效点
    '''
    ...
    '''
    形变一致性约束 (standard_constraint)
    功能：在时间零点强制形变场保持几何不变性，确保t=0时刻形变场恒等映射，避免漂移
    '''
    ...
    def _plane_regulation(self):
        ...
        for grids in multi_res_grids:
            if len(grids) == 3:
                ...
                # time_grids =  [0,1,3]   # 选择空间平面（XY,XZ,YZ）
                time_grids =  [0,1,3,6,7,8,10,11,13,14,17,18,19,20,23,24,26,27]   # 选择空间平面（XY,XZ,YZ,X,Y,Z,XY,Z,XZ,Y,Y,Z,YZ,X,X,Z,X,Y）
    ...
    def _time_regulation(self):
        ...
        for grids in multi_res_grids:
            if len(grids) == 3:
                ...
                time_grids =[2, 4, 5, 9, 12, 15, 16, 21, 22, 25]   # 选择时空平面（ZT,YT,XT,T,T,T,XT,T,YT,ZT）
            for grid_id in time_grids:
                total += compute_plane_smoothness(grids[grid_id])
        return total
    ...
    def _l1_regulation(self):
        ...
        for grids in multi_res_grids:
            if len(grids) == 3:
                ...
                # spatiotemporal_grids = [2, 4, 5]    # 时空网格
                spatiotemporal_grids =[2, 4, 5, 9, 12, 15, 16, 21, 22, 25]   # 选择时空平面（ZT,YT,XT,T,T,T,XT,T,YT,ZT）
